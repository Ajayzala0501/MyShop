...
public class ManageCutomerInvoiceDetailsController {
	...
	public ResponseEntity<ResponseMessageClass<Object>> addNewCustomerInvoiceDetails(@RequestBody CustomerInvoiceDetailsEntity detailsEntity,
		HttpServletRequest request) {
		Registration re = InfoClass.getCurrentUser(request);
		if (re != null) {
			CustomerInvoiceDetailsEntity invDetails = customerInvoiceDetailsService.addNewCustomerInvoiceDetails(detailsEntity, re);
			if (invDetails != null) {
				return ResponseEntity.status(HttpStatus.OK).body(
						new ResponseMessageClass<Object>("New Invoice Details Added Successfully", HttpStatus.OK, "success"));
