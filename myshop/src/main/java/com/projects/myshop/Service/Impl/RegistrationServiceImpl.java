...
//			EmailDetails details = new EmailDetails(model.getEmail(),)
		...
		String res = emailService.sendMailWithAttachment(new EmailDetails(checkEmail.getEmail(), msgBody, "Verify Your E-mail Address"));
		return res;
	}
	
	
	public String applicationURL(Token token, HttpServletRequest request,String url) {
		return "http://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+url+token.getToken();
	}
